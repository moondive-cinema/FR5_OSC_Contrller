    ...
    def _check_preset_format(self):
        ...
        with self.db_lock:
            if not self.db_data or "home" not in self.db_data:
                print(f"[V05 경고] ⚠️ '{DB_FILE}' 비어있음. 'Record-Arm'으로 생성하세요.")
                return
            ...
            if "joint_val" not in home_entry or not home_entry.get("joint_val"):
                 print("="*60)
                 print(f"[V05 경고] ⚠️ 'joint_val' 없음 → MoveCart 폴백 (플립 가능)")
                 print(f"  [권장] migrate_presets_v4.py 실행")
                 print("="*60)
            else:
                 print(f"[V05 확인] ✅ 'joint_val' 지원 확인")
