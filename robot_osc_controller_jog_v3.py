...
# -*- coding: utf-8 -*-
"""
FR5 OSC JOG — Prod build (no dryrun) + Companion feedback
...
- Companion/TouchoOSC 등으로 피드백 송신: /ui/vel/{10|20|40|60|95} = 0/1, /ui/vel/value = float
- 시작 기본 속도 40%, 종료 시 모든 LED OFF
"""
...
print(f"[DBG] SDK_ROOT = {SDK_ROOT}")
if SDK_ROOT not in sys.path:
    sys.path.append(SDK_ROOT)
print(f"[DBG] sys.path head = {sys.path[:3]}")

# ---------------- SDK 임포트 ----------------
try:
    from Robot import RPC as FRRobot
    print("[DBG] ✅ SDK import 성공 (from Robot import RPC as FRRobot)")
except Exception as e:
    print("[ERR] 🛑 SDK import 실패:", e)
    traceback.print_exc()
    sys.exit(1)
...
# Companion/패널로 보낼 피드백 목적지 (LED/라벨 갱신용)
...
JOG_REF_MOVE = 2   # base=2  (joint 테스트: 0)
JOG_REF_STOP = 3   # base=3  (joint 테스트: 1)
...
class FR5JogServer:
    ...
    def _init_robot(self):
        ...
        try:
            ...
            # 전역 속도 비율 (전체 상위 스케일)
            print("[DBG] SetSpeed(30) 호출")
            try:
                rc = self.robot.SetSpeed(30.0)   # 0~100 (%)
    ...
    def _init_osc(self):
        ...
        def _default_log(addr, *args):
            self.rx_count += 1
            print(f"[RX] {addr} {args}  (total={self.rx_count})")
        self.disp.set_default_handler(_default_log)
        print("[DBG] Default handler 등록 완료")
    ...
    def _cb_jog(self, addr, val, axis):
        ...
        try:
            ...
            # START
            nb  = AXIS_NB[axis]
            dir = 1 if val > 0 else 0  # 0=음, 1=양 (매뉴얼 규약)
            print(f"[DBG] Start 요청 → StartJOG({JOG_REF_MOVE}, nb={nb}, dir={dir}, "
